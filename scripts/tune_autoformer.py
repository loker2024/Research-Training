"""Autoformer 超参数搜索 — ETTh1 h96 网格搜索。

Usage:
    python scripts/tune_autoformer.py
    python scripts/tune_autoformer.py --config configs/autoformer_search.json
    python scripts/tune_autoformer.py --datasets ETTh1 --horizons 96 --dry-run
    python scripts/tune_autoformer.py --max-trials 10 --no-skip

Results are saved to:
    test_results/h{horizon}/{dataset}/autoformer/{run_name}_results.npy
    test_results/h{horizon}/{dataset}/autoformer/{run_name}_summary.json
    test_results/h{horizon}/{dataset}/autoformer/autoformer_search_summary.csv
    test_results/h{horizon}/{dataset}/autoformer/autoformer_search_summary.md
"""
...
from models import AutoformerModel, TimeSeriesDataset, Trainer  # noqa: E402
...
# Fixed training settings (not part of the sweep)
DEFAULT_FIXED: dict = {
    "epochs":       25,
    "patience":     5,
    "batch_size":   32,
...
def run_trial(
    ...
    """Train one Autoformer config and return its summary dict."""
    seed = fixed["seed"]
    set_seed(seed)

    run_name = (
        f"{dataset_name}_h{horizon}_autoformer_"
        f"d{trial_params['d_model']}_h{trial_params['n_heads']}_"
        f"enc{trial_params['n_encoder_layers']}_dec{trial_params['n_decoder_layers']}_"
        f"ff{trial_params['d_ff']}_fac{trial_params['factor']}_"
        f"ks{trial_params['kernel_size']}"
    )
    ...
    model = AutoformerModel(
        ...
        n_encoder_layers=trial_params["n_encoder_layers"],
        n_decoder_layers=trial_params["n_decoder_layers"],
        d_ff=trial_params["d_ff"],
        factor=trial_params["factor"],
...
# ---------------------------------------------------------------------------
SUMMARY_COLS = [
    "run_name", "d_model", "n_heads", "n_encoder_layers", "n_decoder_layers",
    "d_ff", "factor", "kernel_size", "dropout", "model_params",
...
def write_summary(summaries: list[dict], output_dir: Path) -> None:
    ...
    # CSV
    csv_path = output_dir / "autoformer_search_summary.csv"
    ...
    # Markdown
    md_path = output_dir / "autoformer_search_summary.md"
    lines = [
        "# Autoformer Hyperparameter Search Summary",
        "",
...
def build_search_grid(space: dict[str, list]) -> list[dict]:
    """Cartesian product of all search dimensions."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Autoformer hyperparameter grid search")
