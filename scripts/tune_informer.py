"""Informer 超参数搜索 — ETTh1 h96 网格搜索。

Usage:
    python scripts/tune_informer.py
    python scripts/tune_informer.py --config configs/informer_search.json
    python scripts/tune_informer.py --datasets ETTh1 --horizons 96 --dry-run
    python scripts/tune_informer.py --max-trials 10 --no-skip

Results are saved to:
    test_results/h{horizon}/{dataset}/informer/{run_name}_results.npy
    test_results/h{horizon}/{dataset}/informer/{run_name}_summary.json
    test_results/h{horizon}/{dataset}/informer/informer_search_summary.csv
    test_results/h{horizon}/{dataset}/informer/informer_search_summary.md
"""
...
from models import InformerModel, TimeSeriesDataset, Trainer  # noqa: E402
...
def run_trial(
    ...
    """Train one Informer config and return its summary dict."""
    seed = fixed["seed"]
    set_seed(seed)

    run_name = (
        f"{dataset_name}_h{horizon}_informer_"
        ...
        f"dp{str(trial_params['dropout']).replace('.', '')}"
    )
...
SUMMARY_COLS = [
    ...
    "d_ff", "factor", "dropout", "model_params",
...
def write_summary(summaries: list[dict], output_dir: Path) -> None:
    ...
    # CSV
    csv_path = output_dir / "informer_search_summary.csv"
    ...
    # Markdown
    md_path = output_dir / "informer_search_summary.md"
    lines = [
        "# Informer Hyperparameter Search Summary",
        "",
...
def main():
    parser = argparse.ArgumentParser(
        description="Informer hyperparameter grid search")
