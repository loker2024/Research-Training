"""PatchTST 超参数搜索 — ETTh1 h96 网格搜索。

Usage:
    python scripts/tune_patchtst.py
    python scripts/tune_patchtst.py --config configs/patchtst_search.json
    python scripts/tune_patchtst.py --datasets ETTh1 --horizons 96 --dry-run
    python scripts/tune_patchtst.py --max-trials 10 --no-skip

Results are saved to:
    test_results/h{horizon}/{dataset}/patchtst/{run_name}_results.npy
    test_results/h{horizon}/{dataset}/patchtst/{run_name}_summary.json
    test_results/h{horizon}/{dataset}/patchtst/patchtst_search_summary.csv
    test_results/h{horizon}/{dataset}/patchtst/patchtst_search_summary.md
"""
...
from models import PatchTSTModel, TimeSeriesDataset, Trainer  # noqa: E402
...
def run_trial(
    ...
    """Train one PatchTST config and return its summary dict."""
    seed = fixed["seed"]
    set_seed(seed)

    run_name = (
        f"{dataset_name}_h{horizon}_patchtst_"
        f"d{trial_params['d_model']}_h{trial_params['n_heads']}_"
        f"l{trial_params['n_layers']}_ff{trial_params['d_ff']}_"
        f"pl{trial_params['patch_len']}_st{trial_params['stride']}_"
...
# ---------------------------------------------------------------------------
SUMMARY_COLS = [
    "run_name", "d_model", "n_heads", "n_layers", "d_ff",
    "patch_len", "stride", "dropout", "model_params",
...
def write_summary(summaries: list[dict], output_dir: Path) -> None:
    ...
    # CSV
    csv_path = output_dir / "patchtst_search_summary.csv"
    ...
    # Markdown
    md_path = output_dir / "patchtst_search_summary.md"
    lines = [
        "# PatchTST Hyperparameter Search Summary",
        "",
...
def main():
    parser = argparse.ArgumentParser(
        description="PatchTST hyperparameter grid search")
