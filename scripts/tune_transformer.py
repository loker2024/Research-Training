"""Transformer hyperparameter tuning via grid search.

Usage:
    python scripts/tune_transformer.py
    python scripts/tune_transformer.py --config configs/transformer_search.json
    python scripts/tune_transformer.py --datasets ETTh1 --horizons 96 --dry-run

Results are saved to:
    test_results/h{horizon}/{dataset}/transformer/{run_name}_results.npy
    test_results/h{horizon}/{dataset}/transformer/{run_name}_summary.json
    test_results/h{horizon}/{dataset}/transformer/transformer_search_summary.csv
    test_results/h{horizon}/{dataset}/transformer/transformer_search_summary.md
"""
...
from models import TransformerModel, TimeSeriesDataset, Trainer  # noqa: E402
...
# The script generates the Cartesian product of all lists.
...
DEFAULT_FIXED: dict = {
    ...
    "batch_size":   32,
    "seed":         216,
    "sample_limit": 0,      # 0 = use full dataset
    "device":       "auto",
    "num_workers":  0,
    "skip_existing": True,
}
...
def run_trial(
    ...
    """Train one Transformer config and return its summary dict."""
    seed = fixed["seed"]
    set_seed(seed)

    run_name = (
        f"{dataset_name}_h{horizon}_transformer_"
        f"d{trial_params['d_model']}_h{trial_params['nhead']}_"
        f"l{trial_params['num_layers']}_ff{trial_params['dim_feedforward']}_"
    ...
    model = TransformerModel(
        ...
        dim_feedforward=trial_params["dim_feedforward"],
...
# ---------------------------------------------------------------------------
SUMMARY_COLS = [
    "run_name", "d_model", "nhead", "num_layers", "dim_feedforward",
    "dropout", "lr", "weight_decay", "model_params",
...
def write_summary(summaries: list[dict], output_dir: Path) -> None:
    ...
    # CSV
    csv_path = output_dir / "transformer_search_summary.csv"
    ...
    # Markdown
    md_path = output_dir / "transformer_search_summary.md"
    lines = [
...
def main():
    parser = argparse.ArgumentParser(
        description="Transformer hyperparameter grid search")
